...
docker build -t postgres-mapepire-proxy:0.1.31 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.31
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.31 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.31 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.31
```
