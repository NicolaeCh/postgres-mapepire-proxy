...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.24 .
```
...
docker build -t postgres-mapepire-proxy:0.1.24 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.24
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.24 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.24 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.24
```
