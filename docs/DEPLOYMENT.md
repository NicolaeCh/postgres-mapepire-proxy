...
docker build -t postgres-mapepire-proxy:0.1.15 .
```
...
Before TypeScript compilation, a successful 0.1.15 build must print all four runtime dependency checks:
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.15
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.15 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.15 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.15
```
