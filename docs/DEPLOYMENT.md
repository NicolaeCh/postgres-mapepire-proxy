...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.20 .
```
...
docker build -t postgres-mapepire-proxy:0.1.20 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.20
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.20 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.20 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.20
```
