...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.25 .
```
...
docker build -t postgres-mapepire-proxy:0.1.25 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.25
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.25 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.25 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.25
```
