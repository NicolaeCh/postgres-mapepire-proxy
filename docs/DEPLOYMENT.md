...
docker build -t postgres-mapepire-proxy:0.1.29 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.29
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.29 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.29 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.29
```
