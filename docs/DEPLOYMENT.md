...
docker build -t postgres-mapepire-proxy:0.1.30 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.30
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.30 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.30 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.30
```
