...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.21 .
```
...
docker build -t postgres-mapepire-proxy:0.1.21 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.21
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.21 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.21 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.21
```
