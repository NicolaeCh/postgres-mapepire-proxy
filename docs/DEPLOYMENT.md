...
docker build -t postgres-mapepire-proxy:0.1.32 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.32
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.32 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.32 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.32
```
