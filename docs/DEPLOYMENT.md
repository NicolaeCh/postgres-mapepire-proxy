...
docker build -t postgres-mapepire-proxy:0.1.23 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.23
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.23 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.23 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.23
```
