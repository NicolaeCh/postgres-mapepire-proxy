...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.16 .
```
...
docker build -t postgres-mapepire-proxy:0.1.16 .
```
...
Before TypeScript compilation, a successful 0.1.16 build must print all four runtime dependency checks:
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.16
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.16 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.16 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.16
```
