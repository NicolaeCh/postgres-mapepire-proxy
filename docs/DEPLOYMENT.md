...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.26 .
```
...
docker build -t postgres-mapepire-proxy:0.1.26 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.26
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.26 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.26 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.26
```
