...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.22 .
```
...
docker build -t postgres-mapepire-proxy:0.1.22 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.22
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.22 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.22 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.22
```
