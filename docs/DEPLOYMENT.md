...
docker build -t postgres-mapepire-proxy:0.1.0 .
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.0 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.0 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.0
```
