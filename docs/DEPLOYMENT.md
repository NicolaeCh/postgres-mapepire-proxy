...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.17 .
```
...
docker build -t postgres-mapepire-proxy:0.1.17 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.17
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.17 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.17 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.17
```
