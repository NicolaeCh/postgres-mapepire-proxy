...
podman build -f Containerfile -t postgres-mapepire-proxy:0.1.27 .
```
...
docker build -t postgres-mapepire-proxy:0.1.27 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.27
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.27 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.27 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.27
```
