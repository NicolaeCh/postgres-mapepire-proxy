...
docker build -t postgres-mapepire-proxy:0.1.34 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.34
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.34 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.34 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.34
```
