...
docker build -t postgres-mapepire-proxy:0.1.33 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.33
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.33 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.33 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.33
```
