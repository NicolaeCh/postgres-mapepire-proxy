...
docker build -t postgres-mapepire-proxy:0.1.28 .
```
...
podman run -d \
  ...
  postgres-mapepire-proxy:0.1.28
```
...
```bash
IMAGE=registry.example.com/db/postgres-mapepire-proxy:0.1.28 \
...
podman manifest push --all postgres-mapepire-proxy:0.1.28 \
  docker://registry.example.com/db/postgres-mapepire-proxy:0.1.28
```
